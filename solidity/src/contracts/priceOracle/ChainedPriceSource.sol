...
import "./BaseTownsqPriceSource.sol";

/**
 * @title ChainedPriceSource
 * @notice An adapter to chain multiple ITownsqPriceSource price feeds into a single price feed
 */
contract ChainedPriceSource is BaseTownsqPriceSource {
    struct ChainedSource {
        ITownsqPriceSource source;
        bytes32 inputAsset;
        string outputAsset;
        uint256 maxPriceAge;
    }
    ...
    ) BaseTownsqPriceSource(_sources[_sources.length - 1].outputAsset) {
        ...
        for (uint256 i = 0; i < _sources.length; i++) {
            require(
                _sources[i].source.priceAvailable(_sources[i].inputAsset) &&
                    keccak256(
                        abi.encodePacked(_sources[i].source.outputAsset())
                    ) ==
                    keccak256(abi.encodePacked(_sources[i].outputAsset)) &&
    ...
    function getPrice(
        ...
        for (uint256 i = 0; i < sources.length; i++) {
            Price memory _sourcePrice = sources[i].source.getPrice(
                sources[i].inputAsset,
                sources[i].maxPriceAge
            );
            if (i == 0) {
                // first element of the chain. seed the price and scale to target precision
                price.price =
                    (_sourcePrice.price * PRICE_PRECISION) /
                    _sourcePrice.precision;
                ...
                price.price =
                    (price.price * _sourcePrice.price) /
                    _sourcePrice.precision;
