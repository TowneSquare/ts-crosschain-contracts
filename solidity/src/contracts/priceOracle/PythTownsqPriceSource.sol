...
/**
 * @title TownsqPriceSource
 */
contract PythTownsqPriceSource is BaseTownsqPriceSource {
    IPyth public pyth;
    ...
    function getPrice(
        bytes32 _asset,
        uint256 _maxAge
    ) public view override returns (Price memory price) {
        ...
        PythStructs.Price memory oraclePrice = pyth.getPriceNoOlderThan(
            assetPythIds[_asset],
            _maxAge
        );
        ...
        uint256 oraclePrecision = uint256(
            10 ** uint64(uint32(-oraclePrice.expo))
        );
        price.price =
            (uint256(uint64(oraclePrice.price)) * price.precision) /
            oraclePrecision;
        price.confidence =
            (uint256(uint64(oraclePrice.conf)) * price.precision) /
            oraclePrecision;
