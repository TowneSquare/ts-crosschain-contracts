...
contract HubInterestUtilities is HubState {
    ...
    function updateAccrualIndices(address assetAddress) public {
        HubSpokeStructs.AccrualIndices
            memory accrualIndices = getCurrentAccrualIndices(assetAddress);
    ...
    function getCurrentAccrualIndices(
        ...
        HubSpokeStructs.StoredVaultAmount memory globalAssetAmounts = _state
            .totalAssets[assetAddress];
        HubSpokeStructs.AccrualIndices
            memory accrualIndices = getInterestAccrualIndices(assetAddress);
        if (
            secondsElapsed != 0 &&
            globalAssetAmounts.amounts.borrowed != 0 &&
            globalAssetAmounts.amounts.deposited != 0
        ) {
            IAssetRegistry.AssetInfo memory assetInfo = getAssetInfo(
                assetAddress
            );
            ...
            (
                ...
                uint256 precision
            ) = assetCalculator.computeSourceInterestFactor(
                    secondsElapsed,
            ...
            accrualIndices.borrowed =
                (accrualIndices.borrowed * borrowInterestFactor) /
                precision;
            accrualIndices.deposited =
                (accrualIndices.deposited * depositInterestFactor) /
                precision;
        }
    ...
    function applyInterest(
        address asset,
        HubSpokeStructs.StoredVaultAmount memory vaultAmount
    ) internal view returns (HubSpokeStructs.DenormalizedVaultAmount memory) {
        return applyInterest(vaultAmount, getCurrentAccrualIndices(asset));
    }

    function applyInterest(
        HubSpokeStructs.StoredVaultAmount memory vaultAmount,
        HubSpokeStructs.AccrualIndices memory indices
    ) internal pure returns (HubSpokeStructs.DenormalizedVaultAmount memory) {
        ...
        return
            HubSpokeStructs.DenormalizedVaultAmount({
                deposited: vaultAmount.amounts.deposited == 0
                    ? 0
                    : (vaultAmount.amounts.deposited * indices.deposited) /
                        vaultAmount.accrualIndices.deposited,
                borrowed: vaultAmount.amounts.borrowed == 0
                    ? 0
                    : (vaultAmount.amounts.borrowed * indices.borrowed) /
                        vaultAmount.accrualIndices.borrowed
            });
    }
}
