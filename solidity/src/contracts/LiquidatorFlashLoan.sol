...
contract LiquidatorFlashLoan is IFlashLoanReceiver, Liquidator {
    ...
    using SwapAmountsOracle for ITownsqPriceOracle;
    ...
    ITownsqPriceOracle public immutable PRICE_ORACLE;
    ...
    constructor(
        ...
        ITownsqPriceOracle _townsqPriceOracle,
        IERC20 _profitToken,
        ...
        require(
            ...
            "LiquidatorFlashLoan: pool address is zero"
        );
        require(
            address(_uniswapRouter) != address(0),
            "LiquidatorFlashLoan: uniswap router address is zero"
        );
        require(
            address(_townsqPriceOracle) != address(0),
            "LiquidatorFlashLoan: townsq price oracle address is zero"
        );

        POOL = _pool;
        ADDRESSES_PROVIDER = IPoolAddressesProvider(_pool.ADDRESSES_PROVIDER());
        UNISWAP_ROUTER = _uniswapRouter;
        PRICE_ORACLE = _townsqPriceOracle;
        ...
        PROFIT_TOKEN_ASSET_ID = hub.getAssetRegistry().getAssetId(
            HUB_CHAIN_ID,
            toWormholeFormat(address(PROFIT_TOKEN))
        );
    ...
    function setProfitReceiver(address _profitReceiver) public onlyOwner {
        require(
            _profitReceiver != address(0),
            "LiquidatorFlashLoan: profit receiver address is zero"
        );
        require(
            _profitReceiver != address(this),
            "LiquidatorFlashLoan: profit receiver address is self (possible griefing attack)"
        );
    ...
    function setMaxSlippage(uint256 _maxSlippage) public onlyOwner {
        require(
            _maxSlippage <= MAX_SLIPPAGE_PRECISION,
            "LiquidatorFlashLoan: max slippage too high"
        );
    ...
    function liquidation(
    ...
    ) public virtual override onlyLiquidator {
        ...
        for (uint256 i = 0; i < input.assets.length; i++) {
            ...
            if (asset.repaidAmount > 0) {
                address hubChainAssetAddress = fromWormholeFormat(
                    assetRegistry.getAssetAddress(asset.assetId, HUB_CHAIN_ID)
                );
    ...
    function executeOperation(
        ...
        ILiquidationCalculator.LiquidationInput memory input = abi.decode(
            params,
        ...
        SwapRepayData[] memory swapRepayData = new SwapRepayData[](
            input.assets.length
        );
        for (uint256 i = 0; i < input.assets.length; i++) {
            swapRepayData[i].asset = input.assets[i].assetId;
            swapRepayData[i].hubChainAddress = fromWormholeFormat(
                assetRegistry.getAssetAddress(
                    input.assets[i].assetId,
                    HUB_CHAIN_ID
                )
            );
            swapRepayData[i].have = IERC20(swapRepayData[i].hubChainAddress)
                .balanceOf(address(this));

            for (
                uint256 assetFlIdx = 0;
                assetFlIdx < assets.length;
                assetFlIdx++
            ) {
                if (assets[assetFlIdx] == swapRepayData[i].hubChainAddress) {
                    swapRepayData[i].need =
                        amounts[assetFlIdx] +
                        premiums[assetFlIdx];
                    break;
                }
            ...
            if (
                swapRepayData[i].hubChainAddress != address(PROFIT_TOKEN) &&
                ...
                IERC20(swapRepayData[i].hubChainAddress).forceApprove(
                ...
                uint256 minAmountOut = (fairAmountOut *
                    (MAX_SLIPPAGE_PRECISION - maxSlippage)) /
                    MAX_SLIPPAGE_PRECISION;
        ...
        for (uint256 i = 0; i < swapRepayData.length; i++) {
            ...
            if (
                ...
                uint256 maxAmountIn = (fairAmountIn *
                    (MAX_SLIPPAGE_PRECISION + maxSlippage)) /
                    MAX_SLIPPAGE_PRECISION;
