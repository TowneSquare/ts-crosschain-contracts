...
interface IWormholeRelayerBase {
    event SendEvent(uint64 indexed sequence, LocalNative deliveryQuote);
...
interface IWormholeRelayerSend is IWormholeRelayerBase {
     ...
     */
    function quoteSolanaDeliveryPrice() external view returns (LocalNative nativePriceQuote);
     ...
     */
    function getRelayerReward() external view returns (LocalNative nativePriceQuote);
...
//When msg.value was not enough to cover delivery costs 
error MsgValueNotEnoughForDeliveryCosts(LocalNative msgValue, LocalNative totalFee);
error RelayerPaymentTooSmall(LocalNative expectedPayment, LocalNative receivedPayment);
