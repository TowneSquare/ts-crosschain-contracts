 ...
 * @dev This contract is ownable only to set the tTownsq contract after deployment
 */
contract rCT is IrCT, Initializable, OwnableUpgradeable {
    ...
    /**
     * @notice Allows the contract owner to set the `tTownsq` address
     * @param _tTownsq Staked TOWNSQ contract address
     */
    function setTTownsq(address _tTownsq) external onlyOwner {
        if (_tTownsq == address(0)) revert InvalidInput();
    ...
    /**
     * @notice Burns rCT tokens. Only callable by the tTOWNSQ contract.
     * @param from Address to receive the rCT
     * @param amount Amount of rCT to mint
     */
    function burn(address from, uint256 amount) external {
        if (msg.sender != tTownsq) revert OnlyTTownsq();
