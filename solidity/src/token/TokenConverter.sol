...
import {ItTOWNSQ} from "./../token/tTOWNSQ.sol";
 ...
 * @notice Contract for converting NEWO to TOWNSQ and claiming rCT
    ...
    PausableUpgradeable
{
    ...
    function initialize(
        ...
        if (
            _ethereumRoot == bytes32("") ||
            _avalancheRoot == bytes32("") ||
            _newoFromEthereum == address(0) ||
            _newoFromAvalanche == address(0)
        ) revert InvalidInput();
    ...
    /**
     * @notice Allows the contract owner to set the `tTownsq` and `rCT` contract addresses
     * @param _tTownsq Staked Townsq contract
     * @param _rCT Rewards Claim Token
     */
    function setTokens(address _tTownsq, address _rCT) external onlyOwner {
        if (_tTownsq == address(0) || _rCT == address(0)) revert InvalidInput();

        tTownsq = ItTOWNSQ(_tTownsq);
        rCT = IrCT(_rCT);
    }
    ...
    function setMerkleRoots(
        ...
        if (_ethereumRoot == bytes32("") || _avalancheRoot == bytes32(""))
            revert InvalidInput();
    ...
    function setDisclaimer(string calldata _disclaimer) public onlyOwner {
        ...
        disclaimerMessageHash = Disclaimer.getDisclaimerMessageHash(
            _disclaimer
        );
    }
    ...
    function rewards(
        address account
    ) public view override returns (ClaimableRewards memory) {
    ...
    function convertWithProof(
        ...
        if (ethereumClaim.rCTCap + avalancheClaim.rCTCap == 0)
            revert InvalidInput(); // revert if neither proof is provided
        ...
        if (ethereumClaim.rCTCap > 0) {
            ...
            __rewards.totalNewo =
                (ethereumClaim.rCTCap * conversionMultiplierPrecision) /
                ethereumClaim.multiplier;
        }
        ...
        if (avalancheClaim.rCTCap > 0) {
            ...
            __rewards.totalNewo +=
                (avalancheClaim.rCTCap * conversionMultiplierPrecision) /
                avalancheClaim.multiplier;
        }

        uint256 totalBurnedNewoAmountInSnapshots = ethereumClaim.amount +
            avalancheClaim.amount;

        __rewards.multiplier =
            (__rewards.rCTCap * conversionMultiplierPrecision) /
            __rewards.totalNewo;
    ...
    /**
     * @notice Transfers NEWO tokens from the caller, burns them, and mints them tTOWNSQ tokens. Also handle any rewards
    ...
    function requireDisclaimerAccepted(
        bytes calldata disclaimerSignature
    ) internal view {
        if (!acceptedDisclaimer[msg.sender]) {
            Disclaimer.requireDisclaimerSignature(
                disclaimerSignature,
                disclaimerMessageHash,
                msg.sender
            );
        }
    }
    ...
    function acceptDisclaimer(string calldata _disclaimer) external {
        if (
            Disclaimer.getDisclaimerMessageHash(_disclaimer) !=
            disclaimerMessageHash
        ) {
    ...
    /**
     * @notice Update storage for rewards burned for the given `user`; only callable by the tTOWNSQ contract
     * @param user The user to update storage for
     * @param amountRewards The amount of rCT burned
     * @param _tTownsq The amount of tTOWNSQ that is unstaked and also contributes to the rCT burning
     */
    function updateBurned(
        address user,
        ...
        if (msg.sender != address(tTownsq)) revert OnlyTTownsq();

        _rewards[user].burned += amountRewards;
        _rewards[user].burnedTtown += _tTownsq;
    }
    ...
    /**
     * @dev Perform NEWO => TOWNSQ conversion
     * @param network The network the NEWO token came from
     * @param amount amount of tokens to convert
     * @param recipient the account to receive tTOWNSQ
     */
    function _convert(
        Network network,
        uint256 amount,
    ...
    function _handleRewards(
        ...
        uint256 rewardAmount = (amountConverted * __rewards.multiplier) /
            conversionMultiplierPrecision;
        uint256 amount = rewardAmount <= remainingClaimable
            ? rewardAmount
            : remainingClaimable;
