...
describe("Hub (unit tests)", () => {
  async function deployHubFixture() {
    ...
    const poolId = 1;
    const pool = await new MockHubPool__factory(user).deploy("townSq USD Coin", "fUSDC", poolId);
  ...
  describe("Direct Operation", () => {
    it("Should not revert when sender is registered to account", async () => {
      ...
      const directOperation = hub.connect(user).directOperation(Action.DepositTsToken, accountId, data);
    ...
    it("Should not revert when sender is delegate to account", async () => {
      ...
      // direct operation
      const data = getRandomBytes(BYTES32_LENGTH + 1 + UINT256_LENGTH);
      const directOperation = hub.connect(delegate).directOperation(Action.DepositTsToken, accountId, data);
      await expect(directOperation).to.not.be.reverted;
    });
    ...
    describe("Deposit TS Token", () => {
      it("Should successfully call deposit ts token", async () => {
        ...
        // verify deposit ts token is called
        await expect(directOperation)
          .to.emit(loanManager, "DepositTsToken")
          .withArgs(loanId, accountId, poolId, delegate.address, fAmount);
      });
    });

    describe("Withdraw TS Token", () => {
      it("Should successfully call withdraw ts token", async () => {
        const { delegate, hub, poolId, loanManager, accountId } = await loadFixture(deployHubFixture);
        ...
        const directOperation = hub.connect(delegate).directOperation(Action.WithdrawTsToken, accountId, data);

        // verify withdraw ts token is called
        await expect(directOperation)
          .to.emit(loanManager, "WithdrawTsToken")
          .withArgs(loanId, accountId, poolId, delegate.address, fAmount);
      });
    });
