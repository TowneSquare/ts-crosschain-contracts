...
describe("LoanManager (unit tests)", () => {
  ...
  async function addPoolsFixture() {
    ...
    // prepare pools
    const usdcPoolId = 1;
    const usdcPool = await new MockHubPool__factory(user).deploy("townSq USD Coin", "fUSDC", usdcPoolId);
    const ethPoolId = 2;
    const ethPool = await new MockHubPool__factory(user).deploy("townSq Ethereum", "fETH", ethPoolId);
  ...
  async function depositFEtherFixture() {
    ...
    // deposit into eth pool
    const depositTsToken = await loanManager
      .connect(hub)
      .depositTsToken(loanId, accountId, pools.ETH.poolId, user.address, depositFAmount);
  ...
  describe("Deposit TS Token", () => {
    it("Should successfully do new deposit ts token", async () => {
      const {
        user,
        loanManager,
        loanManagerAddress,
      ...
      // check events
      const latestBlockTimestamp = await getLatestBlockTimestamp();
      await expect(depositTsToken)
        .to.emit(loanManager, "RewardIndexesUpdated")
        .withArgs(poolId, 0, 0, latestBlockTimestamp);
      await expect(depositTsToken).to.emit(pool, "BurnTsToken").withArgs(user.address, depositFAmount);
      const loanManagerLogic = await ethers.getContractAt("LoanManagerLogic", loanManagerAddress);
      await expect(depositTsToken).to.emit(loanManagerLogic, "DepositTsToken").withArgs(loanId, poolId, depositFAmount);
    ...
    it("Should successfully increase deposit with deposit ts token", async () => {
      const {
        user,
        hub,
        loanManager,
      ...
      const depositTsToken = await loanManager
        .connect(hub)
        .depositTsToken(loanId, accountId, poolId, user.address, depositFAmount);
    ...
    it("Should successfully do second deposit with deposit ts token", async () => {
      const {
        user,
        hub,
        loanManager,
    ...
    it("Should fail to deposit ts token when user loan is unknown", async () => {
      ...
      const depositTsToken = loanManager
        .connect(hub)
        .depositTsToken(loanId, accountId, pools.ETH.poolId, user.address, depositFAmount);
      await expect(depositTsToken).to.be.revertedWithCustomError(loanManager, "UnknownUserLoan").withArgs(loanId);
    });

    it("Should fail to deposit ts token when user is not owner", async () => {
    ...
    it("Should fail to deposit ts token when loan type deprecated", async () => {
    ...
    it("Should fail to deposit ts token when loan pool unknown", async () => {
    ...
    it("Should fail to deposit ts token when loan pool deprecated", async () => {
    ...
    it("Should fail to deposit ts token when loan pool collateral cap is exceeded", async () => {
      ...
      const depositTsToken = loanManager
        .connect(hub)
        .depositTsToken(loanId, accountId, poolId, user.address, depositFAmount);
      await expect(depositTsToken).to.be.revertedWithCustomError(loanManager, "CollateralCapReached").withArgs(poolId);

      // deposit when collateral cap okay
      nodeOutputData.price = BigInt(3000e18);
      await oracleManager.setNodeOutput(poolId, tokenDecimals, nodeOutputData);
      await loanManager.connect(hub).depositTsToken(loanId, accountId, poolId, user.address, depositFAmount);
    });

    it("Should fail to deposit ts token when sender is not hub", async () => {
      const { user, loanManager, pools, loanId, accountId } = await loadFixture(createUserLoanFixture);
  ...
  describe("Withdraw TS Token", () => {
    it("Should successfully fully withdraw ts token", async () => {
      ...
      // withdraw from eth pool
      const withdrawFAmount = depositFAmount;
      const withdrawTsToken = await loanManager
        .connect(hub)
        .withdrawTsToken(loanId, accountId, poolId, user.address, withdrawFAmount);

      // check events
      const latestBlockTimestamp = await getLatestBlockTimestamp();
      await expect(withdrawTsToken).to.emit(pool, "PreparePoolForWithdrawTsToken");
      await expect(withdrawTsToken)
        .to.emit(loanManager, "RewardIndexesUpdated")
        .withArgs(poolId, 0, 0, latestBlockTimestamp);
      await expect(withdrawTsToken).to.emit(pool, "MintTsToken").withArgs(user.address, withdrawFAmount);
      const loanManagerLogic = await ethers.getContractAt("LoanManagerLogic", loanManagerAddress);
      await expect(withdrawTsToken)
        .to.emit(loanManagerLogic, "WithdrawTsToken")
    ...
    it("Should successfully partially withdraw ts token", async () => {
      const { user, hub, loanManager, loanManagerAddress, loanTypeId, pools, loanId, accountId, depositFAmount } =
        await loadFixture(depositEtherFixture);
    ...
    it("Should fail to withdraw ts token when user loan is unknown", async () => {
      const { user, hub, loanManager, pools, accountId } = await loadFixture(createUserLoanFixture);
      ...
      // withdraw ts token
      const withdrawFAmount = BigInt(0.5e18);
      const withdrawTsToken = loanManager
        .connect(hub)
        .withdrawTsToken(loanId, accountId, pools.ETH.poolId, user.address, withdrawFAmount);
      await expect(withdrawTsToken).to.be.revertedWithCustomError(loanManager, "UnknownUserLoan").withArgs(loanId);
    });

    it("Should fail to withdraw ts token when user is not owner", async () => {
      const { user, hub, loanManager, pools, loanId } = await loadFixture(createUserLoanFixture);
    ...
    it("Should fail to withdraw ts token when user loan doesn't have the collateral", async () => {
    ...
    it("Should fail to withdraw ts token when user loan doesn't have sufficient collateral", async () => {
      ...
      const withdrawTsToken = loanManager
        .connect(hub)
        .withdrawTsToken(loanId, accountId, pools.ETH.poolId, user.address, withdrawFAmount);
      await expect(withdrawTsToken).to.be.revertedWithPanic(PANIC_CODES.ARITHMETIC_OVERFLOW);
    });

    it("Should fail to withdraw ts token when user loan becomes under-collateralised", async () => {
      ...
      // prepare withdraw ts token
      const depositInterestIndex = BigInt(1.05e18);
      await pool.setUpdatedDepositInterestIndex(depositInterestIndex);

      // withdraw ts token when becomes under-collateralised
      let withdrawAmount = BigInt(0.58e18);
      let withdrawFAmount = toFAmount(withdrawAmount, depositInterestIndex);
      const withdrawTsToken = loanManager
        .connect(hub)
      ...
      // withdraw ts token when okay
      withdrawAmount = BigInt(0.57e18);
      withdrawFAmount = toFAmount(withdrawAmount, depositInterestIndex);
      await loanManager.connect(hub).withdrawTsToken(loanId, accountId, poolId, user.address, withdrawFAmount);
    });

    it("Should fail to withdraw ts token when sender is not hub", async () => {
      const { user, loanManager, pools, loanId, accountId } = await loadFixture(createUserLoanFixture);
  ...
  describe("Liquidate", () => {
    it("Should successfully liquidate variable borrow when seizing new borrow and collateral", async () => {
      ...
      await expect(liquidate).to.emit(pools.ETH.pool, "MintTsTokenForFeeRecipient").withArgs(reserveCollateralFAmount);
      const loanManagerLogic = await ethers.getContractAt("LoanManagerLogic", loanManagerAddress);
      await expect(liquidate)
