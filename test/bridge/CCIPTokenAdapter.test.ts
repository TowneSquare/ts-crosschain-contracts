...
describe("CCIPTokenAdapter (unit tests)", () => {
  ...
  async function addSupportedTokenFixture() {
    const { user, admin, unusedUsers, adapter, relayer, bridgeRouter, townSqChainId, ccipChainId, corrAdapterAddress } =
  ...
  describe("Send Messsage", () => {
    ...
    it("Should fail to send message when sender is not bridge router", async () => {
      const { user, adapter, townSqChainId, tokenAddress } = await loadFixture(addSupportedTokenFixture);
    ...
    it("Should fail to send message when immediate finality level", async () => {
      const { adapter, bridgeRouter, townSqChainId, tokenAddress } = await loadFixture(addSupportedTokenFixture);
  ...
  describe("Receive Message", () => {
    it("Should successfuly receive message", async () => {
      const { user, adapter, relayer, townSqChainId, ccipChainId, corrAdapterAddress, token, tokenAddress } =
        await loadFixture(addSupportedTokenFixture);
    ...
    it("Should fail to receive message when chain when not added", async () => {
      const { adapter, relayer, townSqChainId, corrAdapterAddress, tokenAddress } =
        await loadFixture(addSupportedTokenFixture);

      // verify chain unknown
      const ccipChainId = 4;
      expect((await adapter.getChainAdapter(townSqChainId))[0]).to.not.equal(ccipChainId);
    ...
    it("Should fail to receive message when message sender is not corresponding adapter", async () => {
      const { adapter, relayer, townSqChainId, ccipChainId, tokenAddress } =
        await loadFixture(addSupportedTokenFixture);
    ...
    it("Should fail to receive message when no destination token amount", async () => {
      const { adapter, relayer, townSqChainId, corrAdapterAddress, ccipChainId, tokenAddress } =
        await loadFixture(addSupportedTokenFixture);
    ...
    it("Should fail to receive message when token unsupported", async () => {
      const { adapter, relayer, townSqChainId, corrAdapterAddress, ccipChainId } =
        await loadFixture(addSupportedTokenFixture);
    ...
    it("Should fail to receive message when amounts differ", async () => {
      ...
      const message = getMessage(townSqChainId, tokenAddress, recipientAddress, metadataAmount);
      const messageId = getRandomBytes(BYTES32_LENGTH);
      const ccipMessage: CCIPMessageReceived = {
        messageId,
        sourceChainSelector: BigInt(ccipChainId),
    ...
    it("Should fail to receive message when sender is not ccip relayer", async () => {
      const { admin, adapter, townSqChainId, ccipChainId, corrAdapterAddress, tokenAddress } =
        await loadFixture(addSupportedTokenFixture);
