...
describe("WormholeDataAdapter (unit tests)", () => {
  ...
  async function addChainFixture() {
    ...
    await adapter.connect(admin).addChain(townSqChainId, wormholeChainId, corrAdapterAddress);
  ...
  describe("Add Chain", () => {
    it("Should successfully add chain", async () => {
      const { adapter, townSqChainId, wormholeChainId, corrAdapterAddress } = await loadFixture(addChainFixture);

      // verfy added
      expect(await adapter.isChainAvailable(townSqChainId)).to.be.true;
      expect(await adapter.getChainAdapter(townSqChainId)).to.be.eql([BigInt(wormholeChainId), corrAdapterAddress]);
    });
    ...
    it("Should fail to add chain when sender is not manager", async () => {
      ...
      // add chain
      const addChain = adapter.connect(user).addChain(townSqChainId, wormholeChainId, corrAdapterAddress);
    ...
    it("Should fail to add chain when already added", async () => {
      ...
      // add chain
      const addChain = adapter.connect(admin).addChain(townSqChainId, wormholeChainId, corrAdapterAddress);
  ...
  describe("Get Send Fee", () => {
    it("Should successfuly get send fee", async () => {
      const { adapter, wormhole, townSqChainId } = await loadFixture(addChainFixture);
  ...
  describe("Send Messsage", () => {
    it("Should successfuly send immediate finality message", async () => {
      const { adapter, bridgeRouter, refundAddress, relayer, townSqChainId, wormholeChainId, corrAdapterAddress } =
  ...
  describe("Receive Message", () => {
    it("Should successfuly receive message", async () => {
      const { adapter, relayer, bridgeRouter, townSqChainId, wormholeChainId, corrAdapterAddress } =
        await loadFixture(addChainFixture);
    ...
    it("Should fail to receive message when chain when not added", async () => {
      const { adapter, relayer, townSqChainId, corrAdapterAddress } = await loadFixture(addChainFixture);
    ...
    it("Should fail to receive message when message sender is not corresponding adapter", async () => {
      const { adapter, relayer, townSqChainId, wormholeChainId } = await loadFixture(addChainFixture);
    ...
    it("Should fail to receive message when sender is not wormhole relayer", async () => {
      const { admin, adapter, townSqChainId, wormholeChainId, corrAdapterAddress } = await loadFixture(addChainFixture);
