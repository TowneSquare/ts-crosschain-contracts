...
describe("WormholeCCTPAdapter (unit tests)", () => {
  ...
  async function addChainFixture() {
    ...
    await adapter.connect(admin).addChain(townSqChainId, wormholeChainId, cctpDomainId, corrAdapterAddress);
  ...
  describe("Add Chain", () => {
    it("Should successfully add chain", async () => {
      const { adapter, townSqChainId, wormholeChainId, cctpDomainId, corrAdapterAddress } =
        await loadFixture(addChainFixture);

      // verfy added
      expect(await adapter.isChainAvailable(townSqChainId)).to.be.true;
      expect(await adapter.getChainAdapter(townSqChainId)).to.be.eql([
    ...
    it("Should fail to add chain when sender is not manager", async () => {
      ...
      // add chain
      const addChain = adapter.connect(user).addChain(townSqChainId, wormholeChainId, cctpDomainId, corrAdapterAddress);
    ...
    it("Should fail to add chain when already added", async () => {
      ...
      // add chain
      const addChain = adapter
        .connect(admin)
        .addChain(townSqChainId, wormholeChainId, cctpDomainId, corrAdapterAddress);
  ...
  describe("Get Send Fee", () => {
    it("Should successfuly get send fee", async () => {
      const { adapter, wormhole, townSqChainId, circleTokenAddress } = await loadFixture(addChainFixture);
      ...
      const message = getMessage(townSqChainId, circleTokenAddress, recipientAddress, amount);
  ...
  describe("Send Messsage", () => {
    ...
    it("Should fail to send message when chain not added", async () => {
      ...
      // send message
      const sendMessage = bridgeRouter.sendMessage(message, { value: fee });
      await expect(sendMessage).to.be.revertedWithCustomError(adapter, "ChainUnavailable").withArgs(townSqChainId);
    });

    it("Should fail to send message when sender is not bridge router", async () => {
      const { user, adapter, townSqChainId, circleTokenAddress } = await loadFixture(addChainFixture);
    ...
    it("Should fail to send message when immediate finality level", async () => {
      const { adapter, bridgeRouter, townSqChainId, circleTokenAddress } = await loadFixture(addChainFixture);
    ...
    it("Should fail to send message when token invalid", async () => {
      const { adapter, bridgeRouter, townSqChainId } = await loadFixture(addChainFixture);
      ...
      const recipientAddress = getRandomAddress();
      const amount = BigInt(0.1e18);
      const message = getMessage(townSqChainId, tokenAddress, recipientAddress, amount);
      const fee = await adapter.getSendFee(message);
  ...
  describe("Receive Message", () => {
    ...
    it("Should fail to receive message when chain when not added", async () => {
      const { adapter, relayer, townSqChainId, corrAdapterAddress, circleTokenAddress } =
        await loadFixture(addChainFixture);

      // verify chain unknown
      const wormholeChainId = 4;
      expect((await adapter.getChainAdapter(townSqChainId))[0]).to.not.equal(wormholeChainId);
      ...
      await expect(receiveMessage).to.be.revertedWithCustomError(adapter, "ChainUnavailable").withArgs(townSqChainId);
    });

    it("Should fail to receive message when message sender is not corresponding adapter", async () => {
      const { adapter, relayer, townSqChainId, wormholeChainId, cctpDomainId, circleTokenAddress } =
        await loadFixture(addChainFixture);

      // verify sender unknown
      const corrAdapterAddress = convertEVMAddressToGenericAddress(getRandomAddress());
      expect((await adapter.getChainAdapter(townSqChainId))[1]).to.not.equal(corrAdapterAddress);
    ...
    it("Should fail to receive message when sender is not wormhole relayer", async () => {
      const { admin, adapter, townSqChainId, wormholeChainId, corrAdapterAddress, cctpDomainId, circleTokenAddress } =
        await loadFixture(addChainFixture);
    ...
    it("Should fail to receive message when no additional messages", async () => {
      const { adapter, relayer, townSqChainId, wormholeChainId, cctpDomainId, corrAdapterAddress, circleTokenAddress } =
        await loadFixture(addChainFixture);
    ...
    it("Should fail to receive message when amounts differ", async () => {
      ...
      const message = getMessage(townSqChainId, circleTokenAddress, recipientAddress, metadataAmount);
      const messageId = getRandomBytes(BYTES32_LENGTH);
      const nonce = BigInt(8237598235);
      const additionalMessage = ethers.concat([
        getRandomBytes(12),
    ...
    it("Should fail to receive message when token recipient is different", async () => {
      ...
      const message = getMessage(townSqChainId, circleTokenAddress, expectedRecipientAddress, amount);
      const messageId = getRandomBytes(BYTES32_LENGTH);
      const nonce = BigInt(8237598235);
      const additionalMessage = ethers.concat([
        getRandomBytes(12),
