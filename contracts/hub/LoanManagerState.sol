...
contract LoanManagerState is AccessControlDefaultAdminRules {
    ...
    struct LoanPool {
        uint256 collateralUsed; // in ts token
    ...
    struct LoanPoolReward {
        ...
        uint256 collateralSpeed; // rewards distributed each sec, 18dp
        uint256 borrowSpeed; // rewards distributed each sec, 18dp
        uint256 collateralRewardIndex; // total rewards to distribute until now for each collateralized unit, 18dp
        uint256 borrowRewardIndex; // total rewards to distribute until now for each borrowed unit, 18dp
    }
