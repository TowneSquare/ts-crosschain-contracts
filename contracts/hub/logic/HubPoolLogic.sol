...
library HubPoolLogic {
    ...
    function prepareForWithdrawTsToken(HubPoolState.PoolData storage pool) external view {
        if (pool.isDeprecated()) revert DeprecatedPool();
        if (!pool.canMintTsToken()) revert CannotMintTsToken();
    }
    ...
    function canMintTsToken(HubPoolState.PoolData storage poolData) internal view returns (bool) {
        return poolData.configData.canMintTsToken;
    }
