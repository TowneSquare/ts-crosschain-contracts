...
library LoanManagerLogic {
    ...
    event DepositTsToken(bytes32 loanId, uint8 poolId, uint256 fAmount);
    event Withdraw(bytes32 loanId, uint8 poolId, uint256 amount, uint256 fAmount);
    event WithdrawTsToken(bytes32 loanId, uint8 poolId, uint256 fAmount);
    ...
    /// @notice Implements the deposit tsToken function i.e. the user deposits tsToken into the pool,
    /// increasing the collateral balance of its loan.
    /// @dev Emits a DepositTsToken event.
    /// @param userLoans The mapping of the loan ID to user loan including loan type, collateral and borrow details.
    /// @param loanTypes The mapping of the type ID to loan types data including a mapping with the pools' details.
    /// @param pools The mapping of pool IDs to pool contracts.
    /// @param oracleManager The OracleManager contract.
    /// @param params The deposit tsToken parameters including the loan ID, pool ID and fAmount.
    function executeDepositTsToken(
        mapping(bytes32 => LoanManagerState.UserLoan) storage userLoans,
        mapping(uint16 loanTypeId => LoanManagerState.LoanType) storage loanTypes,
        mapping(uint8 => IHubPool) storage pools,
        mapping(bytes32 accountId => mapping(uint8 poolId => LoanManagerState.UserPoolRewards)) storage userPoolRewards,
        IOracleManager oracleManager,
        DataTypes.ExecuteDepositTsTokenParams memory params
    ) external {
        LoanManagerState.UserLoan storage userLoan = userLoans[params.loanId];
        LoanManagerState.LoanType storage loanType = loanTypes[userLoan.loanTypeId];
        LoanManagerState.LoanPool storage loanPool = loanType.pools[params.poolId];
        ...
        // burn ts token from user
        pools[params.poolId].burnTsToken(params.sender, params.fAmount);

        emit DepositTsToken(params.loanId, params.poolId, params.fAmount);
    }
    ...
    /// @notice Implements the withdraw tsToken function i.e. the user withdraws tsToken from the pool,
    /// decreasing the collateral balance of its loan, will receive the tsToken amount.
    /// @dev Emits a WithdrawTsToken event.
    /// @param userLoans The mapping of the loan ID to user loan including loan type, collateral and borrow details.
    /// @param loanTypes The mapping of the type ID to loan types data including a mapping with the pools' details.
    /// @param pools The mapping of pool IDs to pool contracts.
    /// @param oracleManager The OracleManager contract.
    /// @param params The withdraw tsToken parameters including the loan ID, pool ID and fAmount.
    function executeWithdrawTsToken(
        mapping(bytes32 => LoanManagerState.UserLoan) storage userLoans,
        mapping(uint16 loanTypeId => LoanManagerState.LoanType) storage loanTypes,
        mapping(uint8 => IHubPool) storage pools,
        mapping(bytes32 accountId => mapping(uint8 poolId => LoanManagerState.UserPoolRewards)) storage userPoolRewards,
        IOracleManager oracleManager,
        DataTypes.ExecuteWithdrawTsTokenParams memory params
    ) external {
        LoanManagerState.UserLoan storage userLoan = userLoans[params.loanId];
        LoanManagerState.LoanPool storage loanPool = loanTypes[userLoan.loanTypeId].pools[params.poolId];
        ...
        // no need to update the pool as underlying token was already deposited
        pools[params.poolId].preparePoolForWithdrawTsToken();
        ...
        // mint ts token for user
        pools[params.poolId].mintTsToken(params.recipient, params.fAmount);

        emit WithdrawTsToken(params.loanId, params.poolId, params.fAmount);
    }
    ...
    function executeLiquidate(
        ...
        pools[params.colPoolId].mintTsTokenForFeeRecipient(collateralSeized.reserveAmount);
