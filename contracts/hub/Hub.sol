...
contract Hub is BridgeMessenger {
    ...
    function directOperation(Messages.Action action, bytes32 accountId, bytes memory data) external nonReentrant {
        ...
        uint256 index = 0;
        if (action == Messages.Action.DepositTsToken) {
            ...
            loanManager.depositTsToken(loanId, accountId, poolId, msg.sender, fAmount);
        } else if (action == Messages.Action.WithdrawTsToken) {
            bytes32 loanId = data.toBytes32(index);
            index += 32;
            uint8 poolId = data.toUint8(index);
            index += 1;
            ...
            loanManager.withdrawTsToken(loanId, accountId, poolId, msg.sender, fAmount);
