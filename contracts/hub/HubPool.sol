...
abstract contract HubPool is ReentrancyGuard, IHubPool, HubPoolState, ERC20FlashMint {
    ...
    uint8 internal tsTokenDecimals;
    ...
    constructor(
        ...
        address loanManager,
        uint8 tokenDecimals,
        string memory tsTokenName,
        string memory tsTokenSymbol,
        uint8 poolId,
        PoolData memory pooldata,
        IOracleManager oracleManager
    ) HubPoolState(admin, poolId, pooldata, oracleManager) ERC20(tsTokenName, tsTokenSymbol) {
        tsTokenDecimals = tokenDecimals;
    ...
    function preparePoolForWithdrawTsToken() external override onlyRole(LOAN_MANAGER_ROLE) nonReentrant {
        HubPoolLogic.prepareForWithdrawTsToken(_poolData);
    }
    ...
    function mintTsTokenForFeeRecipient(uint256 amount) external override onlyRole(LOAN_MANAGER_ROLE) nonReentrant {
        return _mint(_poolData.feeData.tsTokenFeeRecipient, amount);
    }

    function mintTsToken(address recipient, uint256 amount) external override onlyRole(LOAN_MANAGER_ROLE) nonReentrant {
        return _mint(recipient, amount);
    }

    function burnTsToken(address sender, uint256 amount) external override onlyRole(LOAN_MANAGER_ROLE) nonReentrant {
        return _burn(sender, amount);
    }

    function decimals() public view override returns (uint8) {
        return tsTokenDecimals;
    }
    ...
    function _flashFeeReceiver() internal view override returns (address) {
        return _poolData.feeData.tsTokenFeeRecipient;
    }
}
